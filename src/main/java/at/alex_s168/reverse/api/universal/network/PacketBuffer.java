...
public class PacketBuffer extends ByteBuf
{
    ...
    public String[] readStringArray(int maxLength) throws IOException {

        final ByteArrayInputStream byteArrayInputStream =
                new ByteArrayInputStream(readByteArray(maxLength));
    ...
    // todo: hoffentlich funkt
    public List<? extends Bufferable> readList(Class<? extends Bufferable> clazz) {
        int size = readVarInt();
        List<Bufferable> list = new ArrayList<>();
        for (int i = 0; i < size; i++) {
            try {
                list.add(clazz.newInstance().readData(this));
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
        }
